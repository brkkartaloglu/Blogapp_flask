...
def login():
    ...
    if log_form.validate_on_submit():
        check_user = User.query.filter_by(email=log_form.email.data).first()
        if check_user:
            ...
            error = "No user with this mail address"
    return render_template("login.html", form=log_form, fail=error, logged_in=current_user.is_authenticated)
...
def show_post(post_id):
    ...
    gravatar = Gravatar(app,
                        ...
                        use_ssl=False,
                        base_url=None)
    return render_template("post.html", post=requested_post, logged_in=current_user.is_authenticated, user=current_user, form=comment_form, comments=comments, gravatar=gravatar)
